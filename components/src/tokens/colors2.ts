...
type GeneratedCategories = DotNestedCategories
...
const makeGradients = (accent: unknown, colors: AllColors) => {
  const accentGradient = gradients[accent as Gradient] || colors[accent as Hue]
  const entries = Object.entries({
    ...gradients,
    accent: accentGradient,
  })
  return Object.fromEntries(entries) as Record<Gradient, string>
}
...
const makeMode = (accent: Hue, mode: Mode) => {
  ...
  const allColoursWithRaw = {
    ...allColours,
    ...makeGradients(accent, allColours),
  }

  delete (allColoursWithRaw as any)['raw']
  return allColoursWithRaw as Omit<AllColors, 'raw'>
}
