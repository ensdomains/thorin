...
**Add displayName** to component. This is necessary for playroom to generate snippets from code.
...
To build the components in the components library use:
...
Check if displayName has been added to component. If not, then you may need to convert the snippet to a string. See the [snippets section](#snippets) for more details.
