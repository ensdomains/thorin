...
Wrap the root of your app in a [`ThemeProvider`](https://styled-components.com/docs/advanced) module from [styled-components](https://styled-components.com), import `ThorinGlobalStyles` and declare it as a child of `ThemeProvider` to set global styles. Alternatively, the `createGlobalStyle` module from `styled-components` can be use to replace the `ThorinGlobalStyles` if you prefer to customize your global styles. Set the theme by passing a theme object to `ThemeProvider`. If you do not want the ability to change themes, you can pass in a constant.
